...
const (
	...
	UpdatePaysPartly      = `UPDATE accounts SET price = $1 WHERE accounts.account_type = 'payable' AND accounts.factor_type = 'buy' AND accounts.status = false AND accounts.shop = $2 AND accounts.factor_number = $3 AND accounts.date = $4`
)
